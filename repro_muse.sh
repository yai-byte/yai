#!/usr/bin/env bash
set -uo pipefail  # no -e so we continue after failures
export YAI_LANG=en
ROOT="/workspace"
TMP_HOME="$(mktemp -d)"
...
mkdir -p "$(dirname "$TMP_HOME/api/repos/acme/feedapp/releases/latest")" "$ORIGINAL_ROOT" "$(dirname "$MUSESCORE_CATALOG_PAGE")" "$(dirname "$MUSESCORE_HOME_PAGE")" "$(dirname "$MUSESCORE_LANDING_PAGE")" "$(dirname "$MUSESCORE_DOWNLOAD_PAGE_HTTP")"
...
mkdir -p "$(dirname "$MUSESCORE_DOWNLOAD_PAGE_HTTP")" "$TMP_HOME/musescore.org/download" "$(dirname "$MUSESCORE_LANDING_PAGE")"
...
rc=$?
echo "install musescore exit=$rc"

echo "=== website search msg ==="
grep "website search" "$TMP_HOME/musescore_install.err" || echo "NONE"

echo "=== speculative marker ==="
ls -la "$TMP_HOME/speculative-download-failed" 2>&1 || echo "MISSING"

echo "=== curl log lines containing musescore.test/downloads ==="
grep "musescore.test/downloads" "$TMP_HOME/website-curl.log" || echo "NONE"

echo "=== grep max-time 5 ==="
grep -F "$MUSESCORE_DOWNLOAD_URL" "$TMP_HOME/website-curl.log" | grep -- '--max-time 5' && echo "FOUND" || echo "NOT FOUND"

echo "=== MUSESCORE_DOWNLOAD_URL env check inside fake? ==="
echo "MUSESCORE_DOWNLOAD_URL=$MUSESCORE_DOWNLOAD_URL"

echo "=== ALL curl log lines ==="
wc -l "$TMP_HOME/website-curl.log"
while IFS= read -r line; do echo "LOG: $line"; done < "$TMP_HOME/website-curl.log"
