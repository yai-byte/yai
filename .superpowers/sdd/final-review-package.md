# Final review package — split core.cpp
## Spec
docs/superpowers/specs/2026-07-24-split-core-cpp-design.md
## Plan
docs/superpowers/plans/2026-07-24-split-core-cpp.md
## Progress ledger
# SDD Progress — split core.cpp

Workspace: `/home/fsx/yai` (in-place; `.git` is empty placeholder — no commits, no worktree)

Plan: `docs/superpowers/plans/2026-07-24-split-core-cpp.md`
Spec: `docs/superpowers/specs/2026-07-24-split-core-cpp-design.md`

Global constraints:
- No behavior/signature changes
- Do not split yai.hpp
- Do not commit unless user asks
- Mechanical moves only

## Tasks


Task 1: complete (no commits; review clean). Minors: unused unordered_map + stale banner in core.cpp

Task 2: complete (no commits; review clean)

Task 3: complete (no commits; review clean)

Task 4: complete (smoke link fix + review Approved). Minors: report header still said BLOCKED historically; progress_smoke links process.cpp slightly wider than minimal
...
## Smoke g++ lines
tests/arch_smoke.sh:61:g++ -std=c++17 -Wall -Wextra -Wpedantic -O2 -I"$ROOT/src" \
tests/arch_smoke.sh-62-  -o "$TMP_DIR/arch_test" \
tests/arch_smoke.sh-63-  "$TMP_DIR/arch_test.cpp" "$ROOT/src/arch.cpp" "$ROOT/src/core.cpp" \
tests/arch_smoke.sh-64-  "$ROOT/src/i18n.cpp" "$ROOT/src/process.cpp" \
tests/arch_smoke.sh-65-  "$ROOT/src/download_progress.cpp"
tests/arch_smoke.sh-66-
--
tests/progress_smoke.sh:106:g++ -std=c++17 -Wall -Wextra -Wpedantic -O2 -I"$ROOT/src" \
tests/progress_smoke.sh-107-  -o "$TMP_DIR/progress_test" \
tests/progress_smoke.sh-108-  "$TMP_DIR/progress_test.cpp" \
tests/progress_smoke.sh-109-  "$ROOT/src/core.cpp" \
tests/progress_smoke.sh-110-  "$ROOT/src/download_progress.cpp" \
tests/progress_smoke.sh-111-  "$ROOT/src/i18n.cpp" \

## core.cpp (full thin remainder)
#include "yai.hpp"
...
## i18n.cpp head+symbols
7:namespace {
160:} // namespace
162:Language current_language() {
192:bool use_chinese() {
196:std::string tr(const std::string& msgid) {
205:std::string tr_format(
...
## download_progress.cpp symbols
5:std::string format_byte_count(std::uintmax_t bytes) {
139:namespace {
293:} // namespace
381:namespace {
383:struct DownloadProgressSnapshot {
482:} // namespace
484:void render_download_progress(
507:void clear_download_progress(std::size_t& last_width) {

## Unused include check in core
#include "yai.hpp"
