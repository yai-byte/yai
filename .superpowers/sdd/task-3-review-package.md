...
clean:
	rm -f $(TARGET)
## Makefile diff
--- .superpowers/sdd/Makefile.pre-task3	2026-07-24 21:59:54.885288551 +0800
+++ Makefile	2026-07-24 22:02:02.819281685 +0800
@@ -10,6 +10,7 @@
 	src/commands.cpp \
 	src/commands_doctor.cpp \
 	src/core.cpp \
...
## Checks
progress APIs in download_progress:
5:std::string format_byte_count(std::uintmax_t bytes) {
295:std::uintmax_t download_progress_downloaded_bytes(const fs::path& part) {
484:void render_download_progress(
507:void clear_download_progress(std::size_t& last_width) {
must not be in core:
(none — good)
core seam:
257:bool output_has_fuse_error(const std::string& output) {
268:void ensure_directory(const fs::path& path) {
341:    ensure_directory(to.parent_path());
435:    ensure_directory(config_dir_path());
anon namespace in download_progress:
139:namespace {
293:} // namespace
381:namespace {
482:} // namespace
