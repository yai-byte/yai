...
LEFT_DIR="$TMP_HOME/.local/share/yai/apps/leftover-app"
mkdir -p "$LEFT_DIR"
...
# No metadata.json means this directory is a leftover, not an install record.
...
if HOME="$TMP_HOME" "$ROOT/yai" repair leftover-app 2>"$TMP_HOME/repair.err"; then
  echo "FAIL: repair must reject a leftover directory"
  exit 1
fi
grep -q "metadata.json" "$TMP_HOME/repair.err"
grep -q "yai remove leftover-app" "$TMP_HOME/repair.err"
...
grep -q "leftover-app" "$TMP_HOME/doctor.out"
